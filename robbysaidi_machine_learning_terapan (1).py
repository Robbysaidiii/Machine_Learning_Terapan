# -*- coding: utf-8 -*-
"""Robbysaidi_Machine_Learning_Terapan.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Rd-M0RMX3e-ZbuY0huroynTMD8ZZLZuh
...
"""
* `data['Thn']`: Mengakses kolom tahun, yang menunjukkan tahun pengamatan.
* `data['bln']`: Mengakses kolom bulan dari tanggal pengamatan.
* `data['tgl']`: Mengakses kolom tanggal (hari) dari pengamatan.
* `data['temp_min']`: Menyimpan data suhu minimum harian.
* `data['temp_max']`: Menyimpan data suhu maksimum harian.
* `data['temp_rata-rata']`: Menyimpan data suhu rata-rata harian.
* `data['lembab_rata-rata']`: Menyimpan data kelembaban rata-rata harian.
* `data['ch']`: Menyimpan data curah hujan harian.
* `data['cahaya_jam']`: Menyimpan data jumlah jam penyinaran cahaya matahari per hari.

Dengan membuat variabel terpisah untuk setiap kolom, analisis atau visualisasi bisa dilakukan lebih mudah terhadap masing-masing fitur data cuaca.
"""
...
Setelah model Random Forest dibuat dengan parameter default kecuali `n_estimators` dan `random_state`, model dilatih menggunakan data training (`X_train`, `y_train`). Prediksi dilakukan pada data testing (`X_test`), kemudian hasilnya dibandingkan dengan label asli (`y_test`) menggunakan classification report. Report tersebut menampilkan metrik seperti precision, recall, f1-score untuk setiap kategori hujan, memberikan gambaran performa model pada data yang belum pernah dilihat.
---
